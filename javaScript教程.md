...
当对Number做比较时，通过比较运算符得到布尔值

```
2 > 5; // falsse
5 >= 2; //true
7 == 7; //true
由于JavaScript这个设计缺陷，检测使用===比较
false == 0; //true 会自动转换数据类型再比较
false === 0; //false 不会自动转换类型，如数据类型不一致，返回false
NaN === NaN; // false NaN不等于任何值包括他自己
isNaN(NaN); // true 唯一能通过isNaN()函数
1/3 === (1-2/3); // false 因为计算机无法精准表示无限循环小数，要比较两个浮点数是否相等，只能计算它们之差的绝对值，是否小于某个阀值如下
